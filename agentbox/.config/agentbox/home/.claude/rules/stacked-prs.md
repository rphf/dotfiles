...
- Open a PR for a new layer with `gh stack submit`, not `gh pr create`. A PR opened with `gh pr create` gets the right base branch but is not part of the stack on GitHub. If that happened, run `gh stack submit --auto` to link it; it creates nothing when every branch already has a PR.
