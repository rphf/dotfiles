return {
    ...
    config = function()
      require("gitsigns").setup({
        ...
        on_attach = function(bufnr)
          ...
          map("n", "<leader>hi", gitsigns.preview_hunk_inline, { desc = "Preview diff inline" })
          ...
          map("n", "<leader>hq", gitsigns.setqflist, { desc = "File changes in trouble" })
  ...
  --           local function tbl_ensure(t, path)
  --             local parts = type(path) == "string" and vim.split(path, ".", { plain = true }) or path
  --             local current = t
  ...
  --           end
  --
  --           local function tbl_union_extend(target, source)
  --             for k, v in pairs(source) do
  --               if type(v) == "table" and type(target[k]) == "table" then
  --                 local target_is_array = #target[k] > 0
  --                   and not next(vim.tbl_filter(function(k1)
  --                     return type(k1) ~= "number"
  --                   end, vim.tbl_keys(target[k])))
  --                 local source_is_array = #v > 0
  --                   and not next(vim.tbl_filter(function(k2)
  --                     return type(k2) ~= "number"
  --                   end, vim.tbl_keys(v)))
  --
  --                 if target_is_array and source_is_array then
  --                   local seen = {}
  --                   for _, item in ipairs(target[k]) do
  --                     seen[item] = true
  --                   end
  ...
  --           post_layout()
  --         end,
  --         -- Show all lines in the file by default
